...
syn keyword	cStatement	break return continue asm class
syn match   cStatement display "#[a-zA-Z0-9]\+\>"
syn keyword	cLabel		case default
syn keyword	cConditional	if else switch
syn keyword	cRepeat		while for do
...
"integer number, or floating point number without a dot and with "f".
syn case ignore
syn match	cNumbers	display transparent "\<\d" contains=cNumber,cFloat,cOctalError,cOctal
" Same, but without octal error (for comments)
syn match	cNumbersCom	display contained transparent "\<\d\|\.\d" contains=cNumber,cFloat,cOctal
syn match	cNumber		display contained "\d\+\(u\=l\{0,2}\|ll\=u\)\>"
"hex number
syn match	cNumber		display contained "0x\x\+\(u\=l\{0,2}\|ll\=u\)\>"
" Flag the first zero of an octal number as something special
syn match	cOctal		display contained "0o\o\+\(u\=l\{0,2}\|ll\=u\)\>"
"floating point number, with dot, optional exponent
syn match	cFloat		display contained "\d\+\.\d*\(e[-+]\=\d\+\)\=[fl]\="
"floating point number, starting with a dot, optional exponent
syn match	cFloat		display contained "\.\d\+\(e[-+]\=\d\+\)\=[fl]\=\>"
...
" flag an octal number with wrong digits
syn match	cOctalError	display contained "0o\o*[89]\d*"
syn case match

syn region	cCommentL	start="//" skip="\\$" end="$" keepend contains=cTodo,cSpaceError,@Spell
syn region	cComment	matchgroup=cCommentStart start="/\*" end="\*/" contains=cTodo,cCommentStartError,cSpaceError,@Spell fold extend
...
syn keyword	cType		this
syn match cType	 "\<u\=int[0-9]*\>"
syn match cType	 "\<float[0-9]*\>"

syn keyword	cStorageClass extern const
