...
int run_test_suite_dir(const char* dir, bool test_stderr) {
    int result = 0;
    ...
    for (size_t i = 0; i < children.count; ++i) {
        ...
        nob_cmd_append(&cmds, "./main.exe", "--quiet", "--exit-0", "--run", src_sb.items);
        if (!nob_cmd_run(&cmds, .stdout_path = dst_sb.items, .stderr_path = err_sb.items)) {}
        ...
        bool pass = true;
        if (test_stderr) {
            Nob_String_View err_sv = nob_sv_trim(nob_sb_to_sv(err_str));
            const char* err_str_c = nob_temp_sv_to_cstr(err_sv);

            while (expected_chop.count) {
                Nob_String_View expected_line = nob_sv_chop_by_delim(&expected_chop, '\n');
                expected_line = nob_sv_trim_right(expected_line);
                if (expected_line.count == 0) continue;

                const char* expected_line_c = nob_temp_sv_to_cstr(expected_line);
                if (!strstr(err_str_c, expected_line_c)) {
                    pass = false;
                }
            }
        } else {
