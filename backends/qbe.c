...
} LL_Backend_Qbe_Frame;

typedef struct {
	uint32_t count, capacity;
	LL_Backend_Qbe_Frame* items;
} LL_Backend_Qbe_Frame_List;

typedef struct ll_backend_qbe_anon_type_entry {
	LL_Type* value;
	int64_t name_id;
	struct ll_backend_qbe_anon_type_entry* next;
} LL_Backend_Qbe_Anon_Type_Entry;

typedef struct {
	String_Builder o;
	String_Builder typedef_o;
	LL_Backend_Qbe_Frame_List frames;

	LL_Backend_Qbe_Anon_Type_Entry* anon_entries[LL_DEFAULT_MAP_ENTRY_COUNT];
...
void backend_qbe_init(Compiler_Context* cc, LL_Backend_Qbe* b) {
	memset(b, 0, sizeof(*b));
}

void backend_qbe_generate_expression(Compiler_Context* cc, LL_Backend_Qbe* b, String_Builder* o, Ast_Base* expr, bool lvalue);
void backend_qbe_generate_typename(Compiler_Context* cc, LL_Backend_Qbe* b, String_Builder* o, Ast_Base* type, String_View* var_name);
void backend_qbe_generate_typename_impl(Compiler_Context* cc, LL_Backend_Qbe* b, String_Builder* o, LL_Type* type);

typedef struct {
	int64_t name_id;
	LL_Type* type;
} LL_Backend_Qbe_Typedef_Queue_Entry;

typedef struct {
	uint32_t count, capacity;
	LL_Backend_Qbe_Typedef_Queue_Entry* items;
} LL_Backend_Qbe_Typedef_Queue;

bool backend_qbe_write_to_file(Compiler_Context* cc, LL_Backend_Qbe* b, char* filepath) {
	...
	LL_Backend_Qbe_Typedef_Queue queue = { 0 };
	int64_t max_value = 0;

	for (int i = 0; i < LEN(b->anon_entries); ++i) {
		LL_Backend_Qbe_Anon_Type_Entry* current = b->anon_entries[i];

		while (current) {
			arena_da_append(&cc->arena, &queue, ((LL_Backend_Qbe_Typedef_Queue_Entry){ current->name_id, current->value }));
	...
	/* for (;;) { */
		for (int i = 0; i < queue.count; ++i) {
			...
			case LL_TYPE_ARRAY:
				...
				backend_qbe_generate_typename_impl(cc, b, &b->typedef_o, ((LL_Type_Array*)queue.items[i].type)->element_type);
...
int64_t backend_qbe_get_anon_struct_for_type(Compiler_Context* cc, LL_Backend_Qbe* b, LL_Type* type) {
	LL_Backend_Qbe_Anon_Type_Entry* result = NULL;
	String_Builder* o = &b->typedef_o;

	size_t hash = HASH_VALUE_FN(type, MAP_DEFAULT_SEED) % LEN(b->anon_entries);
	LL_Backend_Qbe_Anon_Type_Entry* current = b->anon_entries[hash];
	while (current) {
	...
	if (!result) {
		result = arena_alloc(&cc->arena, sizeof(LL_Backend_Qbe_Anon_Type_Entry));
		...
		case LL_TYPE_ARRAY:
			...
			backend_qbe_generate_typename_impl(cc, b, &b->typedef_o, ((LL_Type_Array*)type)->element_type);
...
void backend_qbe_generate_statement(Compiler_Context* cc, LL_Backend_Qbe* b, String_Builder* o, Ast_Base* stmt) {
	int i;
	...
	case AST_KIND_BLOCK:
		...
		for (i = 0; i < AST_AS(stmt, Ast_Block)->count; ++i) {
			backend_qbe_generate_statement(cc, b, o, AST_AS(stmt, Ast_Block)->items[i]);
			output_str("\n");
		}
		b->indent--;
		for (i = 0; i < b->indent; ++i) {
	...
	case AST_KIND_VARIABLE_DECLARATION: {
		...
		backend_qbe_generate_typename(cc, b, o, var_decl->type, &var_decl->ident->str);
		...
		if (var_decl->initializer) {
			...
			case LL_TYPE_ARRAY:
				...
				/* } */
				output_str(" = ");
				backend_qbe_generate_expression(cc, b, o, var_decl->initializer, false);
				break;
			default:
				output_str(" = ");
				backend_qbe_generate_expression(cc, b, o, var_decl->initializer, false);
	...
	case AST_KIND_FUNCTION_DECLARATION: {
		...
		LL_Backend_Qbe_Frame frame = {
			.locals_count = 0,
		};
		arena_da_append(&cc->arena, &b->frames, frame);

		backend_qbe_generate_typename(cc, b, o, fn_decl->return_type, &fn_decl->ident->str);
		output_str("(");
		output_str(")");
		...
		if (fn_decl->body) {
			output_str(" ");
			backend_qbe_generate_statement(cc, b, o, fn_decl->body);
		} else {
			output_str(";");
		}
			output_str("\n");
	...
	default:
		backend_qbe_generate_expression(cc, b, o, stmt, false);
		output_str(";");
		break;
	}
}

void backend_qbe_generate_root(Compiler_Context* cc, LL_Backend_Qbe* b, Ast_Base* stmt) {
	backend_qbe_generate_statement(cc, b, &b->o, stmt);
}
...
void backend_qbe_generate_expression(Compiler_Context* cc, LL_Backend_Qbe* b, String_Builder* o, Ast_Base* expr, bool lvalue) {
	...
	case AST_KIND_BLOCK: {
		...
		for (i = 0; i < AST_AS(expr, Ast_Block)->count; ++i) {
			backend_qbe_generate_statement(cc, b, o, AST_AS(expr, Ast_Block)->items[i]);
			output_str(";\n");
		}
		output_str("})");
...
#pragma GCC diagnostic ignored "-Wswitch"
		case '-': {
			backend_qbe_generate_expression(cc, b, o, AST_AS(expr, Ast_Operation)->right, false);
			/* assert(false); */
			break;
		}
		case '*': {
...
#pragma GCC diagnostic pop
		...
		backend_qbe_generate_expression(cc, b, o, AST_AS(expr, Ast_Operation)->left, false);
		output_str(")");
	...
	case AST_KIND_CAST: {
		...
		output_str("(");
		output_str("(");
		backend_qbe_generate_typename(cc, b, o, &cast->base, NULL);
		output_str(")");
		backend_qbe_generate_expression(cc, b, o, cast->expr, false);
		output_str(")");
		break;
	}
	case AST_KIND_INVOKE: {
		...
		backend_qbe_generate_expression(cc, b, o, inv->expr, false);
		...
		for (i = 0; i < inv->arguments.count; ++i) {
			...
			backend_qbe_generate_expression(cc, b, o, inv->arguments.items[i], false);
		}
		output_str(")");
	...
	case AST_KIND_ARRAY_INITIALIZER: {
		...
		for (i = 0, k = 0; i < lit->count; ++i, ++k) {
			...
			if (lit->items[i]->kind == AST_KIND_KEY_VALUE) {
				...
				output_str("[");
				backend_qbe_generate_expression(cc, b, o, kv->key, false);
				output_str("] = ");
				backend_qbe_generate_expression(cc, b, o, kv->value, false);
			} else {
				backend_qbe_generate_expression(cc, b, o, lit->items[i], false);
			}
		}
	...
	case AST_KIND_INDEX: {
		...
		backend_qbe_generate_expression(cc, b, o, op->left, false);
		switch (op->left->type->kind) {
		case LL_TYPE_POINTER:
			output_str("[");
			backend_qbe_generate_expression(cc, b, o, op->right, false);
			output_str("]");
			break;
		case LL_TYPE_ARRAY:
			output_str(".val[");
	...
	case AST_KIND_RETURN: {
		...
		if (cf->expr) {
			output_str("({ return ");
			backend_qbe_generate_expression(cc, b, o, cf->expr, false);
			output_str("; })");
		} else {
			output_str("({ return; })");
		}
	...
	case AST_KIND_IF: {
		...
		output_str("if (");
		backend_qbe_generate_expression(cc, b, o, iff->cond, false);
		output_str(")");

		if (iff->body) {
			if (iff->body->kind != AST_KIND_BLOCK) {
				output_str("{\n");
				backend_qbe_generate_expression(cc, b, o, iff->body, false);
				output_str("}");
			} else {
				backend_qbe_generate_expression(cc, b, o, iff->body, false);
			}
	...
	case AST_KIND_FOR: {
		...
		if (loop->init) backend_qbe_generate_statement(cc, b, o, loop->init);
		output_str(";");

		if (loop->cond) {
			backend_qbe_generate_expression(cc, b, o, loop->cond, false);
		}
		output_str(";");

		if (loop->update) {
			backend_qbe_generate_expression(cc, b, o, loop->update, false);
		}
		output_str(")");
		...
		if (loop->body) {
			if (loop->body->kind != AST_KIND_BLOCK) {
				output_str("{\n");
				backend_qbe_generate_statement(cc, b, o, loop->body);
				output_str("}");
			} else {
				backend_qbe_generate_statement(cc, b, o, loop->body);
			}
...
void backend_qbe_generate_typename_impl(Compiler_Context* cc, LL_Backend_Qbe* b, String_Builder* o, LL_Type* type) {
	...
	case LL_TYPE_POINTER:
		...
		backend_qbe_generate_typename_impl(cc, b, o, ((LL_Type_Pointer*)type)->element_type);
		output_str("*");
	   	break;
	case LL_TYPE_ARRAY: {
		int64_t name_id = backend_qbe_get_anon_struct_for_type(cc, b, type);
		...
		/* output_str("struct{"); */
		/* backend_qbe_generate_typename_impl(cc, b, right, ((LL_Type_Array*)type)->element_type); */
...
void backend_qbe_generate_typename(Compiler_Context* cc, LL_Backend_Qbe* b, String_Builder* o, Ast_Base* type, String_View* var_name) {
	backend_qbe_generate_typename_impl(cc, b, o, type->type);
	if (var_name) {
		output_str(" ");
