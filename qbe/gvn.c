	...
	case UIns:
		...
		for (n=0; n<2; n++)
			if (req(i->arg[n], r1))
				i->arg[n] = r2;
		if (t2)
			qbe_adduse(t2, UIns, b, i);
		break;
	case UJmp:
		if (req(b->jmp.arg, r1))
			b->jmp.arg = r2;
		if (t2)
			qbe_adduse(t2, UJmp, b);
		break;
	case UXXX:
		die("unreachable");
	}
...
normins(Fn *fn, Ins *i)
{
	...
	for (n=0; n<2; n++) {
		if (!KWIDE(qbe_argcls(i, n)))
		if (qbe_isconbits(fn, i->arg[n], &v))
		if ((v & 0xffffffff) != v)
			i->arg[n] = qbe_getcon(v & 0xffffffff, fn);
	}
...
negcon(int cls, Con *c)
{
	...
	return qbe_foldint(c, Osub, cls, &z, c);
}
...
assoccon(Fn *fn, Blk *b, Ins *i1)
{
	...
	if (i2->op == Osub && negcon(i2->cls, &c2))
		return;
	if (qbe_foldint(&c, op, i1->cls, &c1, &c2))
		return;
	...
	i1->op = op;
	i1->arg[0] = i2->arg[0];
	i1->arg[1] = qbe_newcon(&c, fn);
	qbe_adduse(&fn->tmp[i1->arg[0].val], UIns, b, i1);
}
...
dedupins(Fn *fn, Blk *b, Ins *i)
{
	...
	normins(fn, i);
	if (i->op == Onop || qbe_pinned(i))
		return;

	assert(!req(i->to, R));
	assoccon(fn, b, i);

	r = qbe_copyref(fn, b, i);
	if (!req(r, R)) {
		killins(fn, i, r);
		return;
	}
...
branchdom(Fn *fn, Blk *bif, Blk *bbr1, Blk *bbr2, Blk *b)
{
	...
	if (b != bif
	&& qbe_dom(bbr1, b)
	&& !qbe_reachesnotvia(fn, bbr2, b, bif))
		return 1;
...
int
qbe_zeroval(Fn *fn, Blk *b, Ref r, int cls, int *z)
{
	Blk *d;
	Ref arg;
	int cls1, eqval;
...
usecls(Use *u, Ref r, int cls)
{
	...
	case UIns:
		...
		if (req(u->u.ins->arg[0], r))
			k = qbe_argcls(u->u.ins, 0);
		if (req(u->u.ins->arg[1], r))
		if (k == Kx || !KWIDE(k))
			k = qbe_argcls(u->u.ins, 1);
		return k == Kx ? cls : k;
	case UPhi:
		if (req(u->u.phi->to, R))
...
dedupjmp(Fn *fn, Blk *b)
{
	...
	z = 0;
	if (b->s1 == b->s2
	|| qbe_isconbits(fn, b->jmp.arg, &v)
	|| qbe_zeroval(fn, b, b->jmp.arg, Kw, &z)) {
		if (v == 0 || z)
			b->s1 = b->s2;
...
rebuildcfg(Fn *fn)
{
	...
	nblk = fn->nblk;
	rpo = qbe_emalloc(nblk * sizeof rpo[0]);
...
qbe_gvn(Fn *fn)
{
	...
	con01[0] = qbe_getcon(0, fn);
	con01[1] = qbe_getcon(1, fn);
	...
	gvntbln = nins + nins/2;
	gvntbl = qbe_emalloc(gvntbln * sizeof gvntbl[0]);
	for (n=0; n<fn->nblk; n++) {
		b = fn->rpo[n];
		dedupphi(fn, b);
		for (i=b->ins; i<&b->ins[b->nins]; i++)
