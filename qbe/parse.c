...
tmpref(char *v)
{
	...
	if (tmphcap/2 <= curf->ntmp-Tmp0) {
		...
		tmph = qbe_emalloc(tmphcap * sizeof tmph[0]);
		for (t=Tmp0; t<curf->ntmp; t++) {
			i = qbe_hash(curf->tmp[t].name) & (tmphcap-1);
			for (; tmph[i]; i=(i+1) & (tmphcap-1))
				;
			tmph[i] = t;
		}
	}
	i = qbe_hash(v) & (tmphcap-1);
	for (; tmph[i]; i=(i+1) & (tmphcap-1)) {
		t = tmph[i];
...
parsefn(Lnk *lnk)
{
	...
	curf->nmem = 0;
	curf->nblk = nblk;
	curf->rpo = qbe_vnew(nblk, sizeof curf->rpo[0], PFn);
...
qbe_parse(FILE *f, char *path, void dbgfile(char *), void data(Dat *), void func(Fn *))
{
	...
	thead = Txxx;
	ntyp = 0;
	typ = qbe_vnew(0, sizeof typ[0], PHeap);
	for (;;) {
		lnk = (Lnk){0};
		switch (parselnk(&lnk)) {
		default:
