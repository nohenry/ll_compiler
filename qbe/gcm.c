...
earlyins(Fn *fn, Blk *b, Ins *i)
{
	...
	if (fn->rpo[b0]->depth < fn->rpo[b1]->depth) {
		assert(qbe_dom(fn->rpo[b0], fn->rpo[b1]));
		b0 = b1;
	}
	return qbe_pinned(i) ? b->id : b0;
}
...
lcabid(Fn *fn, uint bid1, uint bid2)
{
	...
	b = qbe_lca(fn->rpo[bid1], fn->rpo[bid2]);
	assert(b);
	return b->id;
}
...
gcmmove(Fn *fn)
{
	...
	nins = 0;
	vins = qbe_vnew(nins, sizeof vins[0], PFn);
	...
	for (t=fn->tmp; t<&fn->tmp[fn->ntmp]; t++) {
		...
		if (t->bid == t->gcmbid)
			continue;
		i = t->def;
		if (qbe_pinned(i) && !canelim(i))
			continue;
		assert(rtype(i->to) == RTmp);
		assert(t == &fn->tmp[i->to.val]);
		if (t->gcmbid != NOBID)
			qbe_addins(&vins, &nins, i);
		*i = (Ins){.op = Onop};
	}
