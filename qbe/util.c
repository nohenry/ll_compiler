...
void
qbe_addins(Ins **pvins, uint *pnins, Ins *i)
{
	if (i->op == Onop)
		return;
	qbe_vgrow(pvins, ++(*pnins));
	(*pvins)[(*pnins)-1] = *i;
}

void
qbe_addbins(Blk *b, Ins **pvins, uint *pnins)
{
	Ins *i;

	for (i=b->ins; i<&b->ins[b->nins]; i++)
		qbe_addins(pvins, pnins, i);
}
...
void
qbe_igroup(Blk *b, Ins *i, Ins **i0, Ins **i1)
{
	Ins *ib, *ie;
...
qbe_cmpwlneg(int op)
{
	if (INRANGE(op, Ocmpw, Ocmpw1))
		return qbe_cmpneg(op - Ocmpw) + Ocmpw;
	if (INRANGE(op, Ocmpl, Ocmpl1))
		return qbe_cmpneg(op - Ocmpl) + Ocmpl;
...
int
qbe_addcon(Con *c0, Con *c1, int m)
{
...
int
qbe_isconbits(Fn *fn, Ref r, int64_t *v)
{
	Con *c;
...
void
qbe_runmatch(uchar *code, Num *tn, Ref ref, Ref *var)
{
	Ref stkbuf[20], *stk;
	uchar *s, *pc;
	int bc, i;
