...
qbe_narrowpars(Fn *fn)
{
	...
	nins = b->nins + npar;
	ins = qbe_vnew(nins, sizeof ins[0], PFn);
	qbe_icpy(ins, b->ins, npar);
	qbe_icpy(ins + 2*npar, b->ins+npar, b->nins-npar);
	b->ins = ins;
	b->nins = nins;
...
qbe_copyref(Fn *fn, Blk *b, Ins *i)
{
	...
	&& req(i->arg[1], CON_Z)
	&& qbe_zeroval(fn, b, i->arg[0], qbe_argcls(i, 0), &z))
		return con01[optab[i->op].eqval^z^1];

	/* redundant and mask */
	if (i->op == Oand
	&& qbe_isconbits(fn, i->arg[1], &v)
	&& (v > 0 && ((v+1) & v) == 0)
...
Ref
qbe_phicopyref(Fn *fn, Blk *b, Phi *p)
{
	Blk *d, **s;
	Phi *p1;
	uint n, c;
