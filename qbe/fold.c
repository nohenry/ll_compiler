...
int
qbe_foldint(Con *res, int op, int w, Con *cl, Con *cr)
{
	union {
		int64_t s;
		uint64_t u;
...
foldflt(Con *res, int op, int w, Con *cl, Con *cr)
{
	...
	if (cl->type != CBits || cr->type != CBits)
		qbe_err("invalid address operand for '%s'", optab[op].name);
	*res = (Con){.type = CBits};
...
opfold(int op, int cls, Con *cl, Con *cr, Fn *fn)
{
	...
	if (cls == Kw || cls == Kl) {
		if (qbe_foldint(&c, op, cls == Kl, cl, cr))
			return R;
	} else
