...
fixarg(Ref *r, int k, Ins *i, Fn *fn)
{
	...
	if (KBASE(k) == 1 && rtype(r0) == RCon) {
		...
		memset(&a, 0, sizeof a);
		a.offset.type = CAddr;
		n = qbe_stashbits(fn->con[r0.val].bits.i, KWIDE(k) ? 8 : 4);
...
adisp(Con *c, Num *tn, Ref r, Fn *fn, int s)
{
	...
	while (!req(r, R)) {
		...
		if (!(match[n] & BIT(Pob)))
			break;
		qbe_runmatch(matcher[Pob], tn, r, v);
		assert(rtype(v[0]) == RCon);
		qbe_addcon(c, &fn->con[v[0].val], s);
		r = v[1];
	}
	return r;
}
...
amatch(Addr *a, Num *tn, Ref r, Fn *fn)
{
	...
	if (amatch(a, tn, rb, fn))
		return qbe_addcon(&a->offset, &co, 1);
	if (!req(ro, R)) {
		assert(rtype(ro) == RCon);
		c = &fn->con[ro.val];
		if (!qbe_addcon(&co, c, 1))
			return 0;
	}
	if (!req(rs, R)) {
...
amd64_isel(Fn *fn)
{
	...
	n = fn->ntmp;
	num = qbe_emalloc(n * sizeof num[0]);
	for (b=fn->start; b; b=b->link) {
		...
		for (i=&b->ins[b->nins]; i!=b->ins;)
			sel(*--i, num, fn);
		qbe_idup(b, curi, &insb[NIns]-curi);
	}
	free(num);
