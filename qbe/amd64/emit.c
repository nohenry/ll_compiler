...
emitins(Ins i, E *e)
{
	...
	case Osub:
		...
		if (req(i.to, i.arg[1]) && !req(i.arg[0], i.to)) {
			...
			emitins(ineg, e);
			qbe_emitf("add%k %0, %=", &i, e);
			break;
		}
		goto Table;
	case Oneg:
		if (!req(i.to, i.arg[0]))
			qbe_emitf("mov%k %0, %=", &i, e);
		if (KBASE(i.cls) == 0)
			qbe_emitf("neg%k %=", &i, e);
		else
			fprintf(e->f,
	...
	case Odiv:
		 ...
		 * conversion to 2-address in qbe_emitf() would fail */
		if (req(i.to, i.arg[1])) {
			i.arg[1] = TMP(XMM0+15);
			qbe_emitf("mov%k %=, %1", &i, e);
			qbe_emitf("mov%k %0, %=", &i, e);
			i.arg[0] = i.to;
		}
		goto Table;
	case Ocopy:
		...
		&& e->fn->con[i.arg[0].val].type == CBits) {
			...
			if (val >= 0 && val <= UINT32_MAX) {
				qbe_emitf("movl %W0, %W=", &i, e);
				break;
			}
			if (rtype(i.to) == RSlot)
			if (val < INT32_MIN || val > INT32_MAX) {
				qbe_emitf("movl %0, %=", &i, e);
				qbe_emitf("movl %0>>32, 4+%=", &i, e);
				break;
			}
		}
		if (qbe_isreg(i.to)
