...
void print_node(Code* node, uint32_t indent, Oc_Writer* w) {
    ...
    switch (node->kind) {
        ...
        case CODE_KIND_INDEX:
            print_node(CODE_AS(node, Code_Slice)->ptr, indent + 1, w);
            if (CODE_AS(node, Code_Slice)->start)
                print_node(CODE_AS(node, Code_Slice)->start, indent + 1, w);
            break;
        ...
        case CODE_KIND_SLICE:
            ...
            if (CODE_AS(node, Code_Slice)->start)
                print_node(CODE_AS(node, Code_Slice)->start, indent + 1, w);
            if (CODE_AS(node, Code_Slice)->stop)
                print_node(CODE_AS(node, Code_Slice)->stop, indent + 1, w);
            break;
